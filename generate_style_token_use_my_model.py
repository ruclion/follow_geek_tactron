...
'''
f = np.load(os.path.join(save_path, 'style_token_10000.npz'))
...
if __name__ == "__main__":
    ...
    def utt2tok(utt_str):
        ...
        for i in range(64):
            toks[0][i] = data_inp[0][i]
        len_arr = np.asarray([int(data_inp_mask[0])], dtype=np.int32)
    ...
    with tf.Session() as sess:
        ...
        while got_utt != "EXIT":
            ...
            print("Generating ...")
            pred_out = sess.run(model.out_stftm, feed_dict={inp:tok, inp_mask:lenar, speaker:speaker_code, style_token_place_holder:tmp_style_token})
