...
'''
tmp_style_token = np.array([[-0.4341563,  -0.15032549],
 [-0.15308689,  0.2290106 ],
 [-0.16811925, -0.23073697],
 [ 0.41465455, -0.2558865 ],
 [-0.19637585, -0.5389992 ],
 [ 0.14472218,  0.13725115],
 [-0.02565056, -0.11415505],
 [ 0.33593518, -0.17505962],
 [ 0.51640284,  0.04959377],
 [-0.0743617,  -0.05525728]], dtype=np.float32)
'''
f = np.load(os.path.join(save_path, 'style_token_36000.npz'))

tmp_style_token = f['all_style']
...
class TTS(Model):
    ...
    def build(self, inp, inp_mask, speaker, style_token_place_holder = None):
        ...
        with tf.variable_scope("encoder"):
            ...
            with tf.variable_scope("changeToVarible"):
                ...
                self.single_style_token = tf.get_variable('style_token', shape=(styles_kind, style_dim),
                                                          dtype=tf.float32)
            ...
            with tf.variable_scope("pre-net"):
                ...
                self.pre_style_token = pre_style_token
...
with tf.variable_scope("model"):
    model = TTS(r=train_r)
    model.build(inp, inp_mask, speaker, style_token_place_holder)
...
if __name__ == "__main__":
    ...
    def utt2tok(utt_str):
        ...
        speaker = np.array([int(speaker)], dtype=np.int32)
        pre_toks = np.array([[char2id_dic(c) for c in utt_str]], dtype=np.int32)
        toks = np.zeros((1, fixed_txt_len), dtype=np.int32)
        l = len(utt_str)
        for i in range(l):
            toks[0][i] = pre_toks[0][i]
    ...
    with tf.Session() as sess:
        ...
        while got_utt != "EXIT":
            ...
            pred_out, alpha_hjk_img, alpha_style_hjk_img, pre_style_token, style_token = sess.run([model.out_stftm, model.alpha_hjk_img, model.alpha_style_hjk_img, model.pre_style_token, model.style_token], feed_dict={inp:tok, inp_mask:lenar, speaker:speaker_code, style_token_place_holder:copy_style_token})
            
            # de-emphasis defined in audio.py
            print('pre:', style_token)

            print(pre_style_token)
